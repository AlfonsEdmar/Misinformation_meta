...
Lastly, we have a a model where we treat age as a category variable on 4 levels with 1 being children ages 0-5, 2 being ages 5-17, 3 being 18-40, and 4 being 40+. The reference category is the middle ages from 18 to 40.
...
By treating the ages as categories we find a significant difference between adolecents and adults, with adolecents samples having a larger avreage misinformation effect with $\beta=.2$ and a 95% CI(.01, .39). This indicates that we have a lot of uncertainty in this effect and that it could be a spurious relationship and no significant differences between the age categories exists. 
...
Here we look at the 17 studies that manipulated the centrality of the items. It does not seem like any variables in this model significantly impact the misinformation effect outside of the post exposure retention interval with a small negative effect just like we have seen in previous analyses. 

```{r echo=FALSE}
meta_centrality <-
  read_rds(here('output/misinformation_meta_centrality.rds'))
...
```{r echo=FALSE}
meta_centrality_no_acc <-
  read_rds(here('output/misinformation_meta_centrality_no_acc.rds'))

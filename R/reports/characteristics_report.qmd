...
library(tidyverse)
df <- read_csv('misinformation_clean_data.csv')
```
...
summary_country <- df %>% 
  group_by(country) %>%
  summarise(
    number_of_samples = n_distinct(id_record),
...
```{r}
df <- read.csv("misinformation_clean_data.csv")
unique_records <- df %>% distinct(id_record, .keep_all = TRUE)
median(unique_records$publication_year)
sum(unique_records$publication_year >= 2018, na.rm = TRUE)
```
...
sum(population$n_number)
print(population_study)

population_n_total <- df %>%
  distinct(id_study, .keep_all = TRUE) %>%
  group_by(population) %>%
  summarise(total_n = sum(n_total))
print (population_n_total)
...
```{r}
unique_studies <- df %>% distinct(id_study, .keep_all = TRUE)

table(unique_studies$within_between)
```

## Proportional designs

This is how many experiemnts used the proportion of the sample of the accuracy rather than mean recognition/recollection.

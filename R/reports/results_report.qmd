...
Let's first look at the random effects and the heterogeneity estimates. We observe a large amount of heterogeneity in both moderating random effects and overall between study heterogeneity. Like in the initial analysis we see a between record heterogeneity of tau^2 = .21 with an (I^2 = 37%), no heterogeneity attributable across studies within the same record, and then substantial heterogeneity attributable to the control group used within samples nested in studies at tau^2=.15 with an (I^2 = 37%).We also see substantial heterogeneity across event materials(tau^2 = .06), the control type used(tau^2 = .05), recollection test used(tau^2 = .05), and test medium(tau^2 = .04).
...
if(!file.exists("misinformation_meta_leverage.rds")){
...
saveRDS(meta_leverage,"misinformation_meta_leverage.rds")
}else{meta_leverage <-
  readRDS("misinformation_meta_leverage.rds")}

if(!file.exists('meta_resid.rds')){
  meta_resid     <- rma.mv(yi      = yi, 
                          ...
                          ~1|population,
                          ~1|test_type,
                          ~1|test_medium,                                                              ~1|exposure_medium),
                           mods    = 
                         ~postevent_retention_interval
                         + postexposure_retention_interval
                         + preevent_warning
                         ...
                         control = list(
                           ...
                           rel.tol   = 1e-8),
                         verbose = FALSE)
  saveRDS(meta_resid, "meta_resid.rds")
}else{
    meta_resid <- readRDS('meta_resid.rds')
  }
...
```{r}
summary(meta_leverage)
ranef.rma.mv(meta_leverage)

summary(meta_resid)
ranef.rma.mv(meta_resid)
```

Removing potentially influential cases does not impact our results in any meaningful way. The main effects found remains stable and changes in the observed heterogeneity is marginal at best. This speaks to the robustness of our findings, however, it does not mean that our results are unbiased in other areas.

```{r}
if(!file.exists("misinformation_meta_leverage_peese.rds")){
meta_leverage_peese  <- rma.mv(yi     = yi, 
                         V      = vi,
                         random = list(
                          ~1|id_record/id_study/id_control,
                          ~1|event_materials,
...
saveRDS(meta_leverage,"misinformation_meta_leverage_peese.rds")
}else{meta_leverage_peese <-
  readRDS("misinformation_meta_leverage_peese.rds")}

funnel.rma(meta_resid)
```
